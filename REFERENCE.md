...
Full documentation for every `omon` command. For a quick introduction, see [README.md](README.md).
...
Starts a local web server with a dark-themed dashboard. Vanilla HTML/CSS/JS, no build step. Binds to `127.0.0.1` by default (localhost only).

```bash
omon serve                        # http://127.0.0.1:11435
omon serve --port 9000            # custom port
omon serve --bind 0.0.0.0         # expose on LAN (no auth — see SECURITY.md)
```
...
| `--host HOST` | Ollama server address (default: `localhost:11434`) |
